...
@router.get("/read_metadata/{team_id}")
...
@router.options("/read_metadata_duckdb")
...
@router.get("/read_metadata_duckdb")
async def read_metadata_duckdb():
    try:
        con = get_duckdb_connection() #changed zzz
        query = f"""
            SELECT *
...
@router.put("/teams/{team_id}/metadata")
...
@router.delete("/{team_id}/player/{jersey_number}")
...
@router.post("/{team_id}/depth_chart_post")
...
@router.get("/{team_id}/depth_chart_get")
...
@router.get("/hello")
async def hello():
    print("Hello endpoint called!  Version 4")
    return {"message": "Hello World!  Version 4"}

@router.get("/{team_id}/active_players")
async def get_active_players(team_id: str):
    """
    Get a list of active players for a specific team
    """
    try:
        # Get DuckDB connection
        con = get_duckdb_connection()
        
        # Get all player files for the team
        try:
            # Query all player files in the team folder
            player_query = f"""
                SELECT 
                    jersey_number,
                    player_name,
                    defensive_position_one as position
                FROM read_parquet('azure://{CONTAINER_NAME}/teams/team_{team_id}/*.parquet')
                WHERE lower(active) = 'active'
                ORDER BY jersey_number
            """
            player_df = con.execute(player_query).fetchdf()
            
            # Convert to list of dictionaries
            active_players = player_df.to_dict(orient='records')
            
            # Get team name from metadata
            try:
                metadata_query = f"""
                    SELECT team_name
                    FROM read_parquet('azure://{CONTAINER_NAME}/metadata/{team_id}.parquet')
                """
                metadata_df = con.execute(metadata_query).fetchdf()
                team_name = metadata_df.iloc[0]['team_name'] if not metadata_df.empty else "Unknown Team"
            except:
        ...
        logger.error(f"Error retrieving active players for team {team_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving active players for team {team_id}: {str(e)}"
        )
